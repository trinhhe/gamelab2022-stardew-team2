...
namespace Curse_of_the_Abyss
{
    class Dynamite : Sprite
    {
        ...
        {
            name = "dynamite";
            position = new Rectangle(x, y, 8, 36);
            this.taken = !barrel;
            this.barrel = barrel;
            this.level = level;
        ...
        public override void Update(List<Sprite> sprites, GameTime gametime)
        {
            ...
            {
                level.toAdd.Add(new Explosion(new Rectangle(position.X-30,position.Y-30,position.Width+60,position.Height+60)));
                remove = true;
            }
        }
            ...
            switch (s.name)
            {
                case ("waterplayer"):
                    ...
                    if (barrel)
                    {
                        ...
                        {
                            level.toAdd.Add(new Dynamite(player.position.X + player.position.Width - 1, player.position.Y + 20, false, level, player));
                        }
                    }
                    else
                    {
